...
//  i = (int)((((float)tick_time->tm_hour * 60.0f) + (float)tick_time->tm_min) / ((23.0f*60.0f)+59.0f) * STATUS_BAR_PXL_WIDTH);
 ...
 // snprintf(s_buffer3, 20, "x=%d, y=%d\r\ns=%d", x,y,tick_time->tm_sec );
...
//  angle = a * 45;
 ...
 // for (angle = a*45 ; angle < (a+1)*45 ; angle++)
//    y = cos_lookup(angle) * radius;
//    x = sin_lookup(angle) * radius;
